	...
	for (int b_i = blockId_start_x; b_i < block_end_x; ++b_i) {
		for (int b_j = blockId_start_y; b_j < block_end_y; ++b_j) {
			...
			for (int k = start_k; k < end_k && k < num_of_vertices; ++k) {
				...
				if (block_internal_end_x > num_of_vertices)	block_internal_end_x = num_of_vertices;
				if (block_internal_end_y > num_of_vertices) block_internal_end_y = num_of_vertices;
...
void *BLOCK_FLOYD_WARSHALL(void *input) {
	...
	for (int r = 0; r < Round; ++r) {
		fflush(stdout);
		/* Phase 1, no parallel */
		Calculation(num_of_vertices, r, thread_ID, r, r, 1, 1);
		pthread_barrier_wait(&barrier);

		/* Phase 2*/
		Calculation(num_of_vertices, r, thread_ID, r, 0, r, 1);

		Calculation(num_of_vertices, r, thread_ID, r, r + 1, Round - r - 1, 1);

		Calculation(num_of_vertices, r, thread_ID, 0, r, 1, r);

		Calculation(num_of_vertices, r, thread_ID, r + 1, r, 1, Round - r - 1);
		pthread_barrier_wait(&barrier);

		/* Phase 3*/
		Calculation(num_of_vertices, r, thread_ID, 0, 0, r, r);

		Calculation(num_of_vertices, r, thread_ID, 0, r + 1, Round - r - 1, r);

		Calculation(num_of_vertices, r, thread_ID, r + 1, 0, r, Round - r - 1);

		Calculation(num_of_vertices, r, thread_ID, r + 1, r + 1, Round - r - 1, Round - r - 1);
		pthread_barrier_wait(&barrier);
	}
